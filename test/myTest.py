...
def train():
    ...
    # image_batch n x h x w x c


    img = cv2.imread('1.jpg')
    img_batch = tf.constant(np.expand_dims(img, 0), tf.float32)
    boxes = tf.constant([[30, 30, 230, 230]])
    labels = tf.constant([1])
    scores = tf.constant([0.6])


    gtboxes_and_label = tf.constant([[30, 30, 230, 230, 1]])
    num_obs = 1
    batch_size = 1
    share_net = "resnet_v1_101"


    with tf.name_scope('draw_gtboxes'):
        gtboxes_in_img = draw_box_with_color(img_batch, tf.reshape(gtboxes_and_label, [-1, 5])[:, :-1],
                                                 text=tf.shape(gtboxes_and_label)[1])
    ...
    # train
    total_loss = tf.losses.get_losses()[0]
    ...
    with tf.Session(config=config) as sess:
        ...
        for step in range(cfgs.MAX_ITERATION):
            ...
            if (step > 0 and step % 1000 == 0) or (step == cfgs.MAX_ITERATION - 1):
                ...
                print(' weights had been saved on ', save_dir)
